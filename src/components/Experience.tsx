      ...
      <div className="container max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="text-center mb-10 sm:mb-16">
          <div className="inline-block bg-white/5 backdrop-blur-md border border-white/10 rounded-full px-4 py-1.5 sm:px-5 sm:py-2 mb-4 sm:mb-6">
            <span className="text-theme-purple font-bold">✦</span> <span className="text-white/80 font-tech tracking-wide text-xs sm:text-sm">EXPERIENCE</span>
          </div>
        ...
        {portfolioData.experiences.length === 0 ? (
          ...
          <div className="relative">
            {/* Timeline line */}
            <div className="absolute left-6 md:left-16 top-3 bottom-3 w-0.5 bg-gradient-to-b from-theme-purple via-blue-400 to-blue-500 hidden md:block"></div>
            
            <div className="space-y-6 sm:space-y-12">
              {portfolioData.experiences.map((exp, index) => (
                <div 
                  key={exp.id}
                  className="glass-morphism border border-white/10 rounded-3xl p-6 sm:p-8 md:pl-24 relative overflow-hidden transition-transform hover:scale-[1.02] duration-300"
                >
                  ...
                  <div className="absolute left-4 top-6 md:left-6 md:top-8 inline-flex items-center justify-center w-8 h-8 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-theme-purple/20 border border-theme-purple/30 lg:left-4">
                    <BriefcaseIcon className="w-4 h-4 md:w-6 md:h-6 text-theme-purple" />
                  </div>
                  
                  <div className="ml-12 md:ml-0">
                    <div className="mb-3 md:mb-4">
                      <h3 className="text-xl md:text-2xl font-tech text-white">
                        {exp.jobTitle}
                      </h3>
                      <p className="text-white/70 font-tech text-sm md:text-base">
                        {exp.company} {exp.location ? `• ${exp.location}` : ''}
                      </p>
                    </div>
                    
                    <div className="flex items-center gap-2 mb-3 md:mb-4 text-white/60 font-tech text-sm md:text-base">
                      <Calendar size={16} className="md:size-18" />
                      <span>
                        {exp.startDate} - {exp.endDate || 'Present'}
                      </span>
                    </div>
                    
                    <p className="text-white/70 font-tech leading-relaxed text-sm md:text-base">
                      {exp.description}
                    </p>
                  </div>
                </div>
